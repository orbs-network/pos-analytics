...
export const DelegatorSearch = () => {
  const { selectedDelegator, delegatorNotFound } = useSelector(
  ...
  return (
    <div
      className={`delegator-search search-input flex-column ${
    ...
    >
      ...
      {delegatorNotFound && (
        <p className="search-input-not-found">{t('delegators.notFound')}</p>
      )}
    </div>
  );
};
